...
```{r echo = TRUE, fig.heigth=4, fig.width=4}
...
The mean of the total number of steps take per day is `r Mean` and the
median is `r Median`.
...
The average number of steps is at its maximum in the `r MaxIndex`th 5 minute interval.
``` {r echo = TRUE}
        NrNA <- sum(is.na(Data[,"steps"]))
```
## Imputing missing values
The dataset has `r NrNA` NA entries in its 'steps' column. Since these missing values may introduce bias
...
Using the modified dataset the mean of the total number of steps take per day is `r MeanM` and the
median is `r MedianM`. Since I have replaced the NA's by their mean for that 5 minute interval the days 
